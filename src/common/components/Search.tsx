...
import { Backdrop, Box, debounce, Portal, Stack, Typography } from '@mui/material'
...
export const Search = () => {
  ...
  const sortResults = (results: ClaimProps[]) => sortBy(results, ['relevance']).reverse()
  const handleFocus = () => {
    setShowResults(true)
  ...
  const handleFetchMore = async () => {
    ...
    try {
      ...
      setSearchResults([...searchResults, ...sortResults(moreSearchedClaims.data)])
  ...
  const getBackdropContent = () => {
    if (isEmpty(searchResults) && isLoading === false) {
      if (isEmpty(searchTerm)) {
        ...
        return (
          <Typography variant="h5" color="primaryContrast" align="center" sx={{ marginTop: 5 }}>
